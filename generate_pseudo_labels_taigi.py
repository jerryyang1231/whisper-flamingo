...
def generate_pseudo_labels(cfg, split):
    ...
    dataset = YTTDTaigiTRSDataset_Pseudo(cfg,
                                        split=split)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"Generating pseudo-labels ({split})"):
            ...
            # # ========== 逐樣本 CER 過濾 ========== 
            # batch_size = len(o_list)
            # for i in range(batch_size):
            #     single_hyp = [o_list[i]] 
            #     single_ref = [l_list[i]]
            #     single_wer, single_cer = wer_cer(hypo=single_hyp, ref=single_ref)

            #     # 若 cer < cfg.cer_threshold，才保留
            #     if single_cer <= cfg.cer_threshold:
            #         results.append({
            #             "id": ids[i],
            #             "pseudo_text": o_list[i],
            #             "cer": single_cer
            #         })
            #     else:
            #         # 直接 skip，不寫入
