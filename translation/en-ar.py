...
def translate_batch(texts):
    try:
        ...
        # 批量生成翻譯文本
        generated_tokens = model.generate(**encoded, forced_bos_token_id=tokenizer.get_lang_id("ar"))
        # 批量解碼
...
output_path = "/share/nas169/jerryyang/corpus/m2m100/librispeech/javanese"  # 替換為實際輸出路徑

datasets = ["train-clean-100", "train-clean-360", "train-other-500", "dev-clean", "dev-other", "test-clean", "test-other"]
for dataset in datasets:
    process_directory_multithreaded(os.path.join(librispeech_path, dataset), os.path.join(output_path, dataset), max_workers=4, batch_size=12)
