...
class SimpleAttention(nn.Module):
    ...
    def forward(self, d_h, c_h, need_att_mask, preatt=None):
        ...
        all_att = torch.zeros((batch_size, max_len, n_v),
                                dtype=dtype,
                                requires_grad=True,
                                device=d_h.device)
        all_score = -torch.ones((batch_size, max_len, n_v),
                                dtype=dtype,
                                requires_grad=True,
                                device=d_h.device) * torch.inf
        # [k, d_dim]
        need_att_d = d_h[need_att_mask]
        # [k, att_dim]
