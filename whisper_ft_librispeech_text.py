...
class WhisperTextModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        # else:
        features, x_t = self.model.encoder(input_ids, translated_text, training=True)

        out = self.model.decoder(dec_input_ids, features, xv=x_t)
...
if __name__ == "__main__":
    ...
    # 使用 LibriSpeech 數據集
    audio_transcript_pair_list = load_librispeech_data_2(cfg.audio_max_length, cfg.text_max_length, include_audio_lens=True)
    ...
    # if cfg.pt_ckpt != '': # load audio-only FT ckpt
    #     checkpoint_root = '/data/sls/scratch/roudi/experiments/LibriSpeech/whisper-audio/checkpoint/'
