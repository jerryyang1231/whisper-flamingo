...
# my command
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-cmn_small_bert.yaml
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-deu_small_bert.yaml
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-cmn+en-deu_small_bert.yaml
...
class WhisperTextModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        out = self.model.decoder(dec_input_ids, features, xt_1=translation_embeddings_1, xt_2=translation_embeddings_2)
    ...
    def validation_step(self, batch, batch_id, dataloader_idx=None):
        ...
        if translation_2 is not None:
            ...
            bert_outputs_2 = self.bert_model(**bert_inputs_2)
            translation_embeddings_2 = bert_outputs_2.last_hidden_state  # [batch_size, seq_len, hidden_size]
            print("translation_embeddings_2.shape :", translation_embeddings_2.shape)
            input("stop")
            features_a = self.model.encoder(input_ids)
            out_at = self.model.decoder(dec_input_ids, features_a, xt_1=translation_embeddings_1, xt_2=translation_embeddings_2)
        else:
            features_a = self.model.encoder(input_ids)
            out_at = self.model.decoder(dec_input_ids, features_a, xt_1=translation_embeddings_1)
