...
import json
from kws import AdaKWSModel  

# my command
# python -u kg-whisper_offline.py config/audio-text/kg-whisper_offline.yaml
...
class MyASRDataset(Dataset):
    def __init__(self, data_list, tokenizer):
        ...
        data_list: list of dict
            [
               {'audio': numpy_array, 'text': str, 'predicted_keywords': [kw1, kw2, ...]},
               ...
            ]
        """
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        wav_data = item['audio']   # numpy array
        text = item['text']        # already normalized
        filtered_keywords = item['predicted_keywords']
        wav_lens = len(wav_data)

        audio = wav_data.flatten().astype(np.float32)
        audio = whisper.pad_or_trim(audio.flatten(), length=160000)
        ...
        dec_input_ids = prompt_ids + [
            self.tokenizer.sot,
            self.tokenizer.special_tokens["<|zh|>"],
...
def adakws_inference(adakws_model, audio_array, keywords, tokenizer, max_length=160000, device='cuda'):
    """
    給一個wav的 numpy array、以及 all_keywords(list of str)，
    回傳偵測出來的filtered_keywords
    """
    # 1. 音訊前處理
    audio = audio_array.flatten().astype(np.float32)
    if max_length is not None:
        audio = whisper.pad_or_trim(audio.flatten(), length=max_length)
    n_mels = 80  # 依照模型大小設定
    mel = whisper.log_mel_spectrogram(audio, n_mels=n_mels)
    mel_tensor = torch.tensor(mel, dtype=torch.float32).unsqueeze(0).to(device)  # shape [1,n_mels,T]

    # 2. 關鍵字tokens & pad
    keyword_tokens = []
    for kw in keywords:
        token_ids = tokenizer.encode(kw)
        keyword_tokens.append(token_ids)
    if len(keyword_tokens) == 0:
        return []

    max_kt_len = max(len(kt) for kt in keyword_tokens)
    padded_tokens = []
    for kt in keyword_tokens:
        pad_len = max_kt_len - len(kt)
        padded_tokens.append(kt + [50257]*pad_len)  # 50257 當作 padding id
    keyword_tensor = torch.tensor(padded_tokens, dtype=torch.long).unsqueeze(0).to(device)  # [1,K,max_kt_len]

    # 3. AdaKWS forward
    with torch.no_grad():
        logits = adakws_model(mel_tensor, keyword_tensor)  # shape=[1,K,2]
        pred = logits.argmax(dim=-1).squeeze(0)  # shape=[K]
    
    filtered = []
    for kw, p in zip(keywords, pred.tolist()):
        if p == 1:
            filtered.append(kw)
    return filtered
...
def main():
    ...
    # 1. 載入 AdaKWS   
    adakws_ckpt = "/share/nas169/jerryyang/whisper-flamingo/models/checkpoints/tmp_best/step-160000-f1=0.8662.ckpt" 
    # 這邊cfg暫時拿asr的，感覺這邊不用特地載kws的cfg
    adakws_model = AdaKWSModel.load_from_checkpoint(adakws_ckpt, strict=False, cfg=cfg, whisper_size="small", lang="zh")
    adakws_model.eval()
    adakws_model.to("cuda" if torch.cuda.is_available() else "cpu")

    # 2. 讀取 JSON 辭典
    with open('mandarin2taibun.json', 'r', encoding='utf-8') as f:
        dictionary = json.load(f)

    # 3. 讀取 HF dataset & 進行 AdaKWS Inference
    dataset_tr = load_dataset("formospeech/yttd_taigi_trs", name='train', split='train')
    # 分train/val/test
    train_set = dataset_tr.filter(lambda sample: sample['id'][:11] not in valid_set_list)
    valid_set = dataset_tr.filter(lambda sample: sample['id'][:11] in valid_set_list)
    test_set = load_dataset("formospeech/yttd_taigi_trs", name='test', split='train')

    # 4. 建立一個list來存「filtered_keywords」的結果
    from whisper.normalizers.basic import BasicTextNormalizer
    ...
    #     text = normalizer(item['text']).replace(" ","")
    #     mandarin_text = item['text_mandarin']
    #     all_keywords = get_all_keywords(mandarin_text, dictionary)
    #     all_keywords = [normalizer(kw).replace(" ","") for kw in all_keywords]
    #     # 用AdaKWS得到filtered keywords
    #     predicted_keywords = adakws_inference(adakws_model, wav_data, all_keywords, tokenizer, max_length=160000, device=device)
    #     train_predicted_list.append({
    #         'audio': wav_data,
    #         'text': text,
    ...
    for item in test_set:
        ...
        # 用AdaKWS得到filtered keywords
        predicted_keywords = adakws_inference(adakws_model, wav_data, all_keywords, tokenizer, max_length=160000, device=device)
        test_predicted_list.append({
            'audio': wav_data,
            'text': text,
            'predicted_keywords': predicted_keywords
        })

    # 5. 建立新的 PyTorch Dataset ，專門讀取 "predicted_list"
    # train_dataset = MyASRDataset(train_predicted_list, tokenizer)
    valid_dataset = MyASRDataset(valid_predicted_list, tokenizer)
    test_dataset = MyASRDataset(test_predicted_list, tokenizer)

    # model.train_dataset = train_dataset
    model.valid_dataset = valid_dataset
    model.test_dataset = test_dataset
    
    # 6. 交給 DataLoader + Trainer
