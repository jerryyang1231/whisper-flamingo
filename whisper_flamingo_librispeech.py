...
from utils import (
    load_wave,
    add_noise,
    WhisperTextCollatorWhithPadding_librispeech_without_bert,
    whisper_optimizer,
...
# my command
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-cmn_tiny.yaml
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-deu_tiny.yaml
# python -u whisper_flamingo_librispeech.py config/audio-text/at_en-cmn+en-deu_tiny.yaml
...
class LibriSpeechTextDataset(Dataset):
    ...
    def process_translation(self, file_id, ref_lang, translation_base_dir):
        translated_text = self.get_translation_text(file_id, translation_base_dir)
        translated_text = self.text_normalizer(translated_text)
        translated_tokens = [self.tokenizer.sot,
                            self.tokenizer.special_tokens["<|{}|>".format(ref_lang)],
                            self.tokenizer.transcribe,
                            self.tokenizer.no_timestamps] + \
                            self.tokenizer.encode(" " + translated_text)
    
        return np.array(translated_tokens[:self.n_ctx]).astype(np.float32)   
    
    def __getitem__(self, id):
        main_lang, ref_lang_1 = cfg.main_lang, cfg.ref_lang_1
        ref_lang_2 = getattr(cfg, 'ref_lang_2', None)  # 檢查是否存在 ref_lang_2
        ...
        dec_input_ids = [self.tokenizer.sot, 
                        self.tokenizer.special_tokens["<|{}|>".format(main_lang)],
                        self.tokenizer.transcribe, 
                        self.tokenizer.no_timestamps] + \
        ...
        # 處理兩個翻譯文本
        translated_text_1 = self.process_translation(file_id, ref_lang_1, self.translation_base_dirs[0])
        translated_text_2 = None
        if ref_lang_2 and len(self.translation_base_dirs) > 1:
            translated_text_2 = self.process_translation(file_id, ref_lang_2, self.translation_base_dirs[1])
...
class WhisperTextModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        translated_text_1 = batch["translated_text_1"].long()
        ...
        if translated_text_2 is not None:
            translated_text_2 = translated_text_2.long()
        ...
        out = self.model.decoder(dec_input_ids, features, xt_1=translated_text_1, xt_2=translated_text_2)
    ...
    def validation_step(self, batch, batch_id, dataloader_idx=None):
        ...
        if any(length > 480000 for length in wav_lens):
            ...
            out_at = self.model.decoder(dec_input_ids, features_a, xt_1=translated_text_1, xt_2=translated_text_2)
    ...
    def train_dataloader(self):
        ...
        return torch.utils.data.DataLoader(dataset,
                          ...
                          collate_fn=WhisperTextCollatorWhithPadding_librispeech_without_bert())
...
if __name__ == "__main__":
    ...
    wandb.init(project="whisper-flamingo",
            config=cfg,
            name="whisper-flamingo en-cmn (batch_size = 16, lr = 3.75e-5, num_train_steps = 160k)",
            # name="whisper-flamingo en-deu (batch_size = 16, lr = 3.75e-5, num_train_steps = 160k)",
            # name="whisper-flamingo en-cmn + en-deu (batch_size = 16, lr = 3.75e-5, num_train_steps = 160k)",
    )
