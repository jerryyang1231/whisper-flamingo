...
# my command
# python -u lilexp.py config/audio-text/at_en-cmn_tiny_bert.yaml
...
class LibriSpeechTextDataset(Dataset):
    def __init__(self, hf_split, tokenizer, sample_rate, model_name, max_length, 
        ...
        # 初始化翻譯模型
        self.translator_tokenizer = AutoTokenizer.from_pretrained('facebook/seamless-m4t-v2-large')
        self.translator_model = AutoModelForSeq2SeqLM.from_pretrained('facebook/seamless-m4t-v2-large')
        self.translator_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.translator_model.to(self.translator_device)
    ...
    def __getitem__(self, id):
        ...
        print("translated_text :", translated_text_1)
        # 使用 jieba 進行斷詞
        zh_words = jieba.lcut(translated_text_1)
        print("zh_words :", zh_words)
        # 提取關鍵詞（您可以根據需要修改提取方式）
        zh_keywords = list(set(zh_words))  # 去重
        print("zh_keywords :", zh_keywords)
        # 將關鍵詞列表轉換為文本，方便後續處理
        zh_keywords_text = ' '.join(zh_keywords)
        print("zh_keywords_text :", zh_keywords_text)
        # 準備翻譯模型的輸入
        translator_inputs = self.translator_tokenizer(
            zh_keywords_text,
            return_tensors="pt",
            padding=True,
            truncation=True,
        ).to(self.translator_device)
        
        # 生成翻譯結果
        translation_outputs = self.translator_model.generate(**translator_inputs, tgt_lang="eng")
        print("translation_outputs :", translation_outputs)
        # 解碼翻譯結果
        en_keywords_text = self.translator_tokenizer.batch_decode(translation_outputs, skip_special_tokens=True)[0]
        print("en_keywords_text :", en_keywords_text)
