...
from utils import (
    load_librispeech_data_from_hugging_face_for_old_data_split,  # 使用專門為 LibriSpeech 設計的數據加載函數
    load_wave,
    add_noise,
...
class LibriSpeechDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        audio_path, text, _ = item  # 正確地解包元組
...
class WhisperModelModule(LightningModule):
    ...
    def val_dataloader_other(self):
        dataset = LibriSpeechDataset(self.__val_dataset_other, 
                                      self.tokenizer, 
                                      SAMPLE_RATE,
                                      self.model_name,
                                      max_length=None,
    ...
    def test_dataloader_other(self):
        dataset = LibriSpeechDataset(self.__test_dataset_other, 
                                      self.tokenizer, 
                                      SAMPLE_RATE,
                                      self.model_name,
                                      max_length=None,
...
wandb.init(project="whisper-flamingo",
           config=cfg,
           name="whisper eval on librispeech ",
)
...
# 使用 LibriSpeech 數據集
audio_transcript_pair_list = load_librispeech_data_from_hugging_face_for_old_data_split(cfg.audio_max_length, cfg.text_max_length, include_audio_lens=True)
