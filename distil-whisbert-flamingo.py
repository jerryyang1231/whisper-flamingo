...
# my command
# python -u distil-whisbert-flamingo.py config/audio-text/distil-whisbert-flamingo.yaml
...
class DistillWhisperModule(LightningModule):
    def __init__(self, cfg, model_name, lang) -> None:
        ...
        # 砍掉 decoder blocks, 只留兩層
        old_blocks = list(self.student.decoder.blocks)
        self.student.decoder.blocks = nn.ModuleList([old_blocks[0], old_blocks[-1]])  
    ...
    def training_step(self, batch, batch_id):
        ...
        alpha = getattr(self.cfg, "alpha", 0.8)
...
def partial_init_student_from_teacher(teacher_model, student_model):
    ...
    # 2) 複製 decoder[0] <- teacher decoder[0]
    student_model.decoder.blocks[0].load_state_dict(
        teacher_model.decoder.blocks[0].state_dict(),
        strict=False
    )

    # 3) 複製 decoder[1] <- teacher decoder[-1]
    student_model.decoder.blocks[1].load_state_dict(
        teacher_model.decoder.blocks[-1].state_dict(),
        strict=False
    )
