...
# my command
# python -u keyword_prompt_analyze.py config/audio-text/keyword_prompt_analyze.yaml
...
class YTTDTaigiTRSDataset(Dataset):
    def __init__(self, split, tokenizer, sample_rate, model_name, max_length, 
                 spec_augment, dictionary, noise_prob=0, noise_fn=None, positive_sample_count=1) -> None:
        super().__init__()
    ...
    def __getitem__(self, id):
        ...
        # 建立正樣本列表
        positive_samples = [word for word in all_keywords if word in text]
        # print("positive_samples :", positive_samples)

        if len(positive_samples) > 0:
            sample_count = min(len(positive_samples), self.positive_sample_count)
            idxs = random.sample(range(len(positive_samples)), sample_count)
            idxs = sorted(idxs)
            selected_positive_samples = [positive_samples[i] for i in idxs]
        else:
            selected_positive_samples = []
        # print("selected_positive_sample :", selected_positive_samples)
        
        # 過濾負樣本：從 all_keywords 中篩選不在 text 中的詞
        # negative_samples = [word for word in all_keywords if word not in text]
        # print("negative_keywords :", negative_keywords)
        
        # 將正樣本和負樣本結合
        # combined_keywords = positive_keywords + negative_keywords
        # print("combined_keywords :", combined_keywords)
        ...
        prompt_ids = [self.tokenizer.sot_prev] + \
                        self.tokenizer.encode(" " + " ".join(selected_positive_samples)) 
...
class WhisperTextModule(LightningModule):
    ...
    def train_dataloader(self):
        dataset = YTTDTaigiTRSDataset('train',
                                      ...
                                      positive_sample_count=cfg.positive_sample_count)  
